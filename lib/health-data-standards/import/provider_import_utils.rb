module ProviderImportUtils
  ...
  def find_or_create_provider(provider_hash, patient=nil)
    provider = HDS::Provider.by_npi(provider_hash[:npi]).first if provider_hash[:npi] && !provider_hash[:npi].empty?
    unless provider
      if provider_hash[:npi]
        provider = HDS::Provider.create(provider_hash)
        ...
        unless ident_roots.size == 0
          provider ||= HDS::Provider.in("cda_identifiers.root" => ident_roots).and.in("cda_identifiers.extension" => ident_extensions).first
        end
        ...
        provider ||= HDS::Provider.where(provider_query).first
        provider ||= HDS::Provider.resolve_provider(provider_hash, patient)
        provider ||= HDS::Provider.create(provider_hash)
      end
    end
    provider
