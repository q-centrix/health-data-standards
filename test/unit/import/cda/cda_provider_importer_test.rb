...
class CDAProviderImporterTest < Minitest::Test
  ...
  def test_import_existing_npi
    provider = HDS::Provider.create(:npi=>"808401234567893")
    assert_equal 1, HDS::Provider.count, "Should be 1 provider in the DB"
    provider = HDS::Provider.first
  ...
  def test_import_non_existant_npi
    assert_equal 0, HDS::Provider.count, "Should be 0 providers in the DB"
    provider = HDS::Provider.first
    ...
    assert_equal 1, providers.length, "Should have found 1 provider in the file"
    assert_equal 1, HDS::Provider.count, "Should be 1 provider in database"
    assert_equal '808401234567893', HDS::Provider.first.npi, "Provider should have  NPI number 808401234567893"
  ...
  def test_import_no_npi
    ...
    providers = @importer.extract_providers(doc)
    assert_equal 1, providers.length, "should have found 1 provider in the file"
    assert_equal HDS::Provider.count , providers.length, "should be as many providers in db as parsed"
    
    providers2 = @importer.extract_providers(doc)
    assert_equal HDS::Provider.count , providers2.length, "should not have created any new providers"    
  ...
  def test_import_of_cda_identifier
    ...
    assert_equal 1, providers.length, "should have found 1 provider in the file"
    assert_equal HDS::Provider.count , providers.length, "should be as many providers in db as parsed"
    assert_equal 1, HDS::Provider.where("cda_identifiers.root" => "Division", "cda_identifiers.extension" => "12345").count
  end
  ...
  def test_import_resolve_provider
    ...
    assert_equal 1, HDS::Provider.count, "Should be 1 providers in the DB"
    ...
    assert_equal HDS::Provider.count , providers.length, "should be 1 provider in the db"
